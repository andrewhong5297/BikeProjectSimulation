...
namespace Unity.Simulation
{
    ...
    public class Configuration
    {
        ...
        public struct SimulationConfiguration
        {
            ...
            private string _storagePathOther;
            ...
            /// <summary>
            /// Returns the full cloud storage path for non-artifact
            public string storagePathOther
            {
                get
                {
                    if (string.IsNullOrEmpty(_storagePathOther))
                    {
                        var temp = storagePath;
                        var paths = new List<string>(temp.Split('/'));
                        for (int i = 0; i < paths.Count; i++)
                        {
                            if (paths[i].Contains(definition_id))
                            {
                                paths.Insert(i + 1, others);
                                break;
                            }
                        }
                        temp = String.Join("/", paths);
                        _storagePathOther = temp;
                    }
                    return _storagePathOther;
                }
            }
        }
        ...
        public T GetAppParams<T>()
        {
            ...
            try
            {
                ...
                if (File.Exists(filePath))
                {
                    ...
                    JsonUtility.FromJsonOverwrite(appConfig, config);
                }
                else
                {
        ...
        /// </summary>
        /// <returns>string of the non-artifact storagePath</returns>
        public string GetStorageBasePathOther()
        {
            return IsSimulationRunningInCloud() ? SimulationConfig.storagePathOther : _persistentDataPath;
        }

        /// <summary>
        /// Returns the full path for the storage which contains base path and the attemptId, for non-artifacts
        /// </summary>
        /// <returns>string of full storage path for non-artifacts</returns>
        public string GetStoragePathOther()
        {
            return Path.Combine(GetStorageBasePathOther(), GetAttemptId());
        }
