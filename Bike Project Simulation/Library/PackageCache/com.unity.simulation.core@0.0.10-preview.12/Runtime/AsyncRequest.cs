...
namespace Unity.Simulation
{
    ...
    public abstract class AsyncRequest
    {
        ...
        public enum ExecutionContext
        {
            ...
            /// <summary>
            /// Invoke the callback using the default execution context.
            /// </summary>
            Default = JobSystem
        }
    ...
    public class AsyncRequest<T> : AsyncRequest, IDisposable
    {
        ...
        public void Start(Func<AsyncRequest<T>, Result> functor = null, ExecutionContext executionContext = ExecutionContext.Default)
        {
            if (functor != null)
                _functors.Enqueue(functor);
        ...
        public void Execute(ExecutionContext executionContext = ExecutionContext.Default)
        {
        ...
        void DispatchWithJobSystem()
        {
            lock (_mutex)
            {
                ...
                var jobHandles = new NativeArray<JobHandle>(functors.Length, Allocator.Temp);

                Interlocked.Add(ref _requestsInFlight, functors.Length);
                for (var i = 0; i < functors.Length; ++i)
                {
                    var f = functors[i];
                    jobHandles[i] = new LambdaJob(() => { InvokeWaitCallback(f); }).Schedule(_previousJobHandle);
                }

                _previousJobHandle = jobHandles.Length > 1 ? JobHandle.CombineDependencies(jobHandles) : jobHandles[0];
        ...
        CustomSampler _profilerSampler = CustomSampler.Create("AsyncRequest.InvokeWaitCallback");

        // Mutex for protecting _previousJobHandle.
        object _mutex = new Mutex();

        // Jobs are chained one after the other to avoid saturating the worker pool.
        static JobHandle _previousJobHandle = new JobHandle();
